...
export default function FaqIndex({ faqs }: { faqs: Faq[] }) {
    const handleDelete = async (id: number) => {
        ...
        router.delete(`/admin/public/faqs/${id}`, {
            onSuccess: () =>
                Swal.fire({
                    title: 'Berhasil',
                    text: 'FAQ dihapus.',
                ...
                <section className="relative overflow-hidden rounded-3xl border border-sky-100/80 bg-white/85 p-6 shadow-sm">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <Button asChild className="bg-sky-600 text-white hover:bg-sky-700">
                            <Link href="/admin/public/faqs/create">Tambah FAQ</Link>
                        </Button>
                    </div>
                </section>
                            ...
                            <tbody>
                                {faqs.map((faq) => (
                                    <tr key={faq.id} className="border-t border-slate-100">
                                        ...
                                        <td className="px-4 py-3">{faq.sort_order}</td>
                                        <td className="px-4 py-3">
                                            <Badge className={faq.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex gap-2">
                                                <Button asChild variant="outline" className="border-sky-200 text-slate-700 hover:bg-sky-50">
                                                    <Link href={`/admin/public/faqs/${faq.id}/edit`}>Edit</Link>
                                                </Button>
                                                <Button
                                                    variant="outline"
