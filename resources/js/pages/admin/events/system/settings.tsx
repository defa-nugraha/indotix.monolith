...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Konfigurasi Event', href: '/admin/events/system/settings' },
];

export default function EventSettings({ setting }: { setting?: Setting | null }) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Konfigurasi Sistem Event" />
            <div className="flex flex-1 flex-col gap-6 bg-[#f6fbff] px-6 py-8">
                <section className="rounded-3xl border border-sky-100/80 bg-white/90 p-6 shadow-sm">
                    <h1 className="text-2xl font-semibold text-slate-900">Konfigurasi Sistem Event</h1>
                    <form
                        className="mt-6 grid gap-3 md:grid-cols-2"
                        onSubmit={(event) => {
                            event.preventDefault();
                            const data = new FormData(event.currentTarget);
                            router.post('/admin/events/system/settings', Object.fromEntries(data.entries()), { preserveScroll: true });
                        }}
                    >
                        <input
