                ...
                <section className="rounded-3xl border border-sky-100/80 bg-white/90 p-6 shadow-sm">
                    <div className="flex items-start justify-between gap-6">
                        <div>
                            ...
                            <p className="text-sm text-slate-500">Pengelola: {event.organizer?.name ?? 'Indotix'}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <Badge className="bg-slate-100 text-slate-600">{event.status}</Badge>
                            <Link
                                href={`/admin/special-programs/${event.id}/edit`}
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                            {['draft', 'pending_review', 'published', 'postponed', 'cancelled', 'completed'].map((status) => (
                                <Button
                                    ...
                                    onClick={() => router.post(`/admin/special-programs/${event.id}/status`, { status })}
                                >
                                    {status}
                                </Button>
                            ))}
                    ...
                    <div className="rounded-3xl border border-sky-100/80 bg-white/90 p-6 shadow-sm">
                        ...
                        <form
                            ...
                            onSubmit={(e) => {
                                ...
                                router.post(`/admin/special-programs/${event.id}/capacity`, Object.fromEntries(data.entries()));
                            }}
                        >
                            <input
                                name="capacity_total"
