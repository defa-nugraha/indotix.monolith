...
export default function SouvenirShow({
    ...
    const unitPrice =
        Number(product.price) + Number(variant?.additional_price ?? 0);
            ...
            },
            {
                ...
                onError: (errors) =>
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
                        text: errors.quantity ?? 'Stok tidak mencukupi.',
                    }),
            },
        );
    };
    ...
    return (
        <PublicLayout categories={navItems}>
            <PublicSeo
                title={`${product.name} - Retail Shop Indotix`}
                description={product.description}
                image={product.images[0]}
                canonicalPath={`/retail-shop/${product.slug ?? product.encrypted_id}`}
                type="product"
                structuredData={{
                ...
                <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
                    ...
                    <div className="rounded-2xl bg-white p-6 shadow-sm">
                        ...
                        <p className="mt-3 text-xs text-slate-500">
                            Kamu bisa membeli lebih dari satu produk retail
                            melalui keranjang.
                        </p>
                    </div>
                </div>
