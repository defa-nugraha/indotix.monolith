...
export default function SpecialProgramSearch({
    ...
    const chips = ['Promo', 'Diskon', 'Flash Sale', 'Liburan', 'Eksklusif', 'Hemat', 'Highlight', 'Early Access', 'Bundling', 'Best Deal', 'Limited'];

    const submitSearch = (event: React.FormEvent) => {
        event.preventDefault();
        router.get('/special-programs', { q: form.q }, { preserveState: true, preserveScroll: true });
    };
                    ...
                    <>
                            ...
                            <div className="relative overflow-hidden rounded-[28px] shadow-lg">
                                ...
                                <div className="absolute left-4 right-4 top-1/2 -translate-y-1/2 text-center text-white sm:left-8 sm:right-8">
                                    <h1 className="text-lg font-semibold sm:text-xl md:text-3xl">
                                        Cari special program favorit? Temukan promo terbaik di INDOTIX
                                    </h1>
                                    <p className="mt-2 text-xs text-white/85 sm:text-sm">
                                        Pilih program, cek periode, dan nikmati benefit terbaik tanpa ribet.
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="relative z-20 rounded-[24px] bg-white p-5 shadow-[0_18px_40px_-18px_rgba(15,23,42,0.35)]">
                                    <form className="grid gap-4 md:grid-cols-[2fr_1.5fr_1fr_auto]" onSubmit={submitSearch}>
                                        <div className="grid gap-2">
                                            <label className="text-xs font-semibold uppercase text-slate-500">Nama program atau kota</label>
                                            <div className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm">
                                                <span className="text-slate-400">🎤</span>
                                                <input
                                                    className="w-full bg-transparent outline-none"
                                                    placeholder="Cari nama program atau lokasi"
                                                    value={form.q}
                                        ...
                                        <div className="grid gap-2">
                                            <label className="text-xs font-semibold uppercase text-slate-500">Tanggal program</label>
                                            <div className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm">
                                                <span className="text-slate-400">📅</span>
                                                <input
                                                    type="date"
                                    ...
                                    <div className="mt-4 text-sm font-semibold text-sky-700">Special program rekomendasi untukmu</div>
                                </div>
                            </div>
                        </section>
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4 sm:gap-6 lg:grid-cols-3 xl:grid-cols-4">
                            {filtered.map((program) => {
                                const detailSlug = program.slug ?? program.encrypted_id;
                                return (
                                <div
                                    key={program.id}
                                ...
                                >
                                    <Link href={`/special-programs/${detailSlug}`} className="relative block h-28 overflow-hidden">
                                        <img
                                            src={
                                                program.image_url ??
                                                fallbackImage ??
                                    ...
                                    <div className="p-3">
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                <h2 className="text-sm font-semibold text-slate-900">{program.title}</h2>
                                                <p className="mt-1 flex items-center gap-1 text-xs text-slate-500">
                                                    <MapPinned className="h-3 w-3 text-sky-500" />
                                                    {program.city_name ?? 'Indonesia'}
                                                </p>
                                                <p className="mt-1 text-xs text-slate-500">{program.start_at ?? 'Segera'}</p>
                                            </div>
                                        </div>
                                        <div className="mt-3 flex items-center justify-between">
                                            <div>
                                                ...
                                                <div className="text-sm font-semibold text-sky-600">
                                                    {program.min_price ? `Rp ${program.min_price.toLocaleString('id-ID')}` : '-'}
                                                </div>
                                            </div>
                                            <Link
                            ...
                            {filtered.length === 0 && (
                                <div className="md:col-span-2 rounded-2xl border border-slate-100 bg-white p-6 text-center text-sm text-slate-500 shadow-sm">
                                    ...
                                    <div className="mt-2">Coba ubah kata kunci untuk menemukan special program terbaik.</div>
                                </div>
                            )}
                        </div>
                    </>
