...
export default function WisataShow({
    ...
    const handleFilter = () => {
        router.get(
            `/wisata/${destination.slug ?? destination.encrypted_id}`,
            { visit_date: visitDate, quantity },
            { preserveState: true },
        );
    };
        ...
        <PublicLayout categories={categories} chips={chips}>
            ...
            <main className="mx-auto w-full max-w-6xl px-4 py-8 md:px-8">
                        ...
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                            <div>
                                <p className="text-sm font-semibold text-slate-900">
                                    Kamu datang dari rekomendasi partner kami{' '}
                                ...
                                <div className="mt-3 grid gap-3">
                                    <div className="grid gap-2">
                                        ...
                                        <div className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm">
                                            ...
                                            <span className="w-full text-slate-700">
                                                {destination.destination_name}
                                            </span>
                                        </div>
                                    </div>
                    ...
                    <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-slate-100 pt-4">
                        ...
                        <div className="rounded-xl bg-slate-50 px-4 py-3 text-right">
                            ...
                            <div className="text-lg font-semibold text-orange-500">
                                Rp{' '}
                                {tickets[0]?.price?.toLocaleString('id-ID') ??
                                    '-'}
                            </div>
                ...
                <div className="sticky top-[120px] z-20 mt-6 flex items-center gap-6 border-b border-slate-200 bg-white/95 px-4 py-3 text-sm font-semibold text-slate-600 shadow-sm backdrop-blur">
                    {['Overview', 'Tickets', 'Location', 'Facilities'].map(
                        (tab) => (
                            <a
                                key={tab}
                        ...
                        <div className="rounded-2xl border border-slate-100 p-4">
                            ...
                            <p className="mt-2 text-sm text-slate-600">
                                {destination.highlights ??
                                    'Destinasi ini cocok untuk liburan singkat yang seru dan berkesan.'}
                            </p>
                        </div>
                        <div className="rounded-2xl border border-slate-100 p-4 md:col-span-2">
                            ...
                            <p className="mt-3 text-sm text-slate-600">
                                {destination.description ??
                                    'Deskripsi destinasi akan tampil di sini.'}
                            </p>
                            <div className="mt-4 grid gap-2 text-sm text-slate-600">
                                <div>
                                    Alamat: {destination.address_full ?? '-'}
                                </div>
                                <div>
                                    Jam buka: {destination.open_time ?? '-'} ·
                                    Jam tutup: {destination.close_time ?? '-'}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                >
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-slate-500">
                            Pilih tiket sesuai kebutuhanmu
                        </span>
                    </div>
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-slate-500">
                                            Sisa kuota: {ticket.available}
                                        </div>
                                    </div>
                                </div>
                                <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600">
                                    <span className="rounded-full bg-slate-100 px-3 py-1">
                                        {ticket.ticket_type === 'grup'
                                            ? 'Grup'
                                            : 'Perorangan'}
                                    </span>
                ...
                <section
                    id="location"
                    className="mt-8 grid gap-4 lg:grid-cols-[1.2fr,1fr]"
                >
                    ...
                    <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
                        ...
                        <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                            Tips: Datang lebih awal untuk menikmati suasana yang
                            lebih santai.
                        </div>
                    </div>
                </section>
                        ...
                        {(destination.facilities ?? []).length > 0 ? (
                            destination.facilities?.map((facility) => {
                                ...
                                return (
                                    <div
                                        key={facility}
                                        className="flex items-center gap-3 rounded-xl border border-slate-100 px-4 py-3 text-sm text-slate-700"
                                    >
