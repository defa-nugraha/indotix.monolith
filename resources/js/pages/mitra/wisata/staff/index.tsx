...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Staff & Role', href: '/mitra/wisata/staff' },
];

export default function MitraWisataStaffIndex({ destination, staff }: Props) {
    const form = useForm({
        name: '',
        email: '',
        role: 'staff_validasi',
    ...
    const deleteStaff = async (staffId: number) => {
        ...
        router.delete(`/mitra/wisata/staff/${staffId}`, {
            preserveScroll: true,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Staff Mitra Wisata" />
            <div className="flex flex-1 flex-col gap-6 bg-[#f6fbff] px-6 py-8">
                <section className="rounded-3xl border border-sky-100/80 bg-white/90 p-6 shadow-sm">
                    <p className="text-xs font-semibold uppercase tracking-[0.3em] text-sky-600">Akses Mitra</p>
                    <h1 className="mt-2 text-2xl font-semibold text-slate-900">
                        Staff {destination.destination_name ?? ''}
                    </h1>
