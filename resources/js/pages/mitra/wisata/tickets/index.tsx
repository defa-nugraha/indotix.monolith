...
export default function MitraWisataTicketsIndex({ destination, tickets }: Props) {
    const handleDelete = async (ticketId: number) => {
        ...
        router.delete(`/mitra/wisata/tickets/${ticketId}`, {
            ...
            preserveScroll: true,
            onSuccess: () =>
                Swal.fire({ icon: 'success', title: 'Terhapus', text: 'Tiket berhasil dihapus.' }),
            onError: () =>
                Swal.fire({ icon: 'error', title: 'Gagal', text: 'Tidak dapat menghapus tiket.' }),
        });
    };
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            ...
                            <h1 className="mt-2 text-2xl font-semibold text-slate-900">
                                Produk Tiket {destination.destination_name ?? ''}
                            </h1>
                        ...
                        <Button
                            ...
                            onClick={() => router.visit('/mitra/wisata/tickets/create')}
                        >
                            Tambah Tiket
                        </Button>
                    </div>
                                    ...
                                    <tr key={ticket.id} className="border-t border-slate-100">
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-xs text-slate-500">
                                                {ticket.ticket_type === 'grup' ? 'Grup' : 'Perorangan'} •{' '}
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-xs text-slate-500">
                                                Kuota harian: {ticket.daily_quota ?? '-'}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            <Badge
                                                className={
                                                    ticket.is_active && !ticket.is_closed
                                                        ? 'bg-emerald-50 text-emerald-700'
                                                        : 'bg-slate-50 text-slate-600'
                                                }
                                            >
                                                {ticket.is_active && !ticket.is_closed ? 'Aktif' : 'Nonaktif'}
                                            </Badge>
                                        </td>
                                        <td className="px-4 py-3">
